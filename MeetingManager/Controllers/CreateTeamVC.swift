    ...
    @IBAction func createPressed(_ sender: Any) {
        if nameField.text != "" && nameAvailable == true {
            if passField.text != "" && confirmField.text == passField.text {
                ...
                let alert = UIAlertController(title: "Ops", message: "Please make sure your passwords match and not Empty", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "Got it", style: .default, handler: nil))
                self.present(alert,animated: true,completion: nil)
            }
        } else {     // name error msg
            let alert = UIAlertController(title: "Ops", message: "Please make sure you are using an available name", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "Got it", style: .default, handler: nil))
            self.present(alert,animated: true,completion: nil)
        }
    }
